    ...
    try:
        ...
        if '/proxy' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'✕Error en el comando /proxy')
            return
        if '/off_proxy' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'✕Error en el comando /off_proxy')
            return
        if '/view_proxy' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'✕Error en el comando /view_proxy')
            return
        if '/dir' in msgText:
            try:
                cmd = str(msgText).split(' ',2)
        ...
        if '/start' in msgText:
            ...
            start_msg+= 'Comandos:\n'
            start_msg+= '➢ Configuración\n'
            start_msg+= ' ┌/account - configurar cuenta moodle.\n'
            start_msg+= ' ├ /host - configurar host de la cuenta.\n'
            start_msg+= ' ├ /repo - configurar ID de la nube.\n'
            start_msg+= ' ├ /cloud - tipo de nube(moodle o cloud).\n'
            start_msg+= ' ├ /uptype - (evidence,draft,blog,calendar).\n'
            start_msg+= ' ├ /zip - tamaño de compresión.\n'
            start_msg+= ' ├ /data_delete - eliminar configuracion.\n'
            start_msg+= ' └ /off_proxy - eliminar proxy.\n'
            start_msg+= '➢ Información\n'
            start_msg+= ' ┌/myuser - mostrar configuración de la cuenta.\n'
            start_msg+= ' ├ /files - mostrar archivos subidos.\n'
            start_msg+= ' ├ /view_proxy - ver proxy configurado.\n'
            start_msg+= ' └ /help - mostrar explicación más detallada sobre los usos del bot.\n\n'
            start_msg+= ' ➢ Version 2.25.0 Basado en la version v7.0 Fixed\n\n'
            ...
            if loged:
                ...
                client.logout()
                bot.editMessageText(message,'Archivo Eliminado ✕')
            else:
                bot.editMessageText(message,'Error:\n\n 1-Revise el estadao de su cuenta\n 2-El servidor esta desabilitado: '+client.path)
            ...
            if isadmin:
                ...
                if loged:
                    try:
                        ...
                        client.logout()
                        bot.editMessageText(message,'Archivos Eliminados Correctamente  ✕')
                    except:
                        bot.editMessageText(message,'Error:\n\n 1-Revise el estadao de su cuenta\n 2-El servidor esta desabilitado: '+client.path)
                    return 
            else:
                bot.editMessage(update.message.chat.id,'✕No Tiene Permiso✕')
            return
...
def main():
    ...
    bot_token = '5377986395:AAFcL0FlddMJj3-6S3WzLEPKlY4lmygYK1U'
